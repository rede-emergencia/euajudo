...
# ============================================================================
# SHELTER INVENTORY SYSTEM (Controle de Estoque para Abrigos)
# ============================================================================

class ShelterInventoryItem(Base):
    """
    Item do inventário/estoque do abrigo.
    Representa uma necessidade contínua (ex: preciso de 100L de água).
    """
    ...
    # Quantidades
    needed_quantity = Column(Float, nullable=False, default=0)  # Quanto precisa
    current_stock = Column(Float, default=0)  # Quanto tem em estoque
    reserved_quantity = Column(Float, default=0)  # Reservado (entregas confirmadas/em trânsito)
    
    # Configurações
    metadata_cache = Column(JSON, default={})  # Atributos específicos (tamanho, tipo, etc)
    priority = Column(String, default="medium")  # urgent, high, medium, low
    min_stock_alert = Column(Float, nullable=True)  # Alerta quando estoque cair abaixo deste valor
    ...
    # Relacionamentos
    shelter = relationship("User", foreign_keys=[shelter_id], backref="inventory_items")
...
class InventoryMovement(Base):
    ...
    Registra todas as entradas e saídas de itens do estoque.
    """
    __tablename__ = "inventory_movements"
    
    id = Column(Integer, primary_key=True, index=True)
    inventory_item_id = Column(Integer, ForeignKey("shelter_inventory_items.id"), nullable=False, index=True)
    
    # Tipo de movimentação
    movement_type = Column(String, nullable=False)  # 'in' (entrada) ou 'out' (saída)
    quantity = Column(Float, nullable=False)
    
    # Origem/Destino
    source_type = Column(String, nullable=False)  
    # Tipos: 'donation' (doação recebida), 'purchase' (compra), 'transfer_in' (transferência recebida),
    #        'distribution' (distribuição/consumo), 'loss' (perda/descarte), 'adjustment' (ajuste manual)
    
    delivery_id = Column(Integer, ForeignKey("deliveries.id"), nullable=True)  # Se veio de uma doação
    
    # Detalhes
    notes = Column(Text, nullable=True)  # Observações sobre a movimentação
    extra_data = Column(JSON, default={})  # Dados adicionais
