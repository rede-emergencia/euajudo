...
export default function ProviderDashboard() {
  ...
  const handleCancelPickup = async (deliveryId) => {
    ...
    try {
      ...
      alert(`✅ Pickup canceled successfully!\n📦 ${quantityReturned} meals returned to stock.`);
      loadData();
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert('✅ Pedido de insumos criado com sucesso!');
      setShowForm(false);
  ...
  const handleCreateBatch = async (e) => {
    ...
    try {
      ...
      alert('✅ Marmitas publicadas com sucesso! Disponíveis para retirada por 4 horas.');
  ...
  const handleCancelBatch = (batchId) => {
    ...
    if (batch && batch.quantidade_reservada > 0) {
      alert('❌ Cannot cancel: reservations already exist for this batch.');
      return;
    }
  ...
  const handleCancelRequest = async (requestId) => {
    ...
    try {
      ...
      alert('❌ Error canceling: ' + (error.response?.data?.detail || error.message));
    }
  };
